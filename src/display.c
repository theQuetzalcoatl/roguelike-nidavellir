...
void display_player_stats(const mob_t player, const uint64_t turns)
{
  ...
  term_move_cursor(6, RUNIC_LINE_POS + 5);
  if(HAS_ARMOR(&player)){
    item_t *armor = player.gear.armor;
    uint8_t max_dur = (SPEC_ATTR(armor, armor_t)->type == CLOTH) ? 1 : SPEC_ATTR(armor, armor_t)->type*10; /* TODO: tie this to the original max durability calc somehow  */
    printf("Armor: %s (%d/%d)", armor->description, SPEC_ATTR(armor, armor_t)->durability, max_dur);
  }
...
void display_to_player_window(const char * const option)
{
  ...
  if(!strcmp(option, "hotkeys")){
    ...
    while(1){
      for(uint8_t row = 0; row < RUNIC_LINE_POS-1; ++row) { /* TODO: extract this and the reverse below to its own function(?) used two times */ 
        term_move_cursor(0, row);
        for(uint8_t col = 0; col < TERMINAL_WIDTH-2; ++col) printf(" ");
      }
